...
if(isset($_REQUEST['login']))
{
    ...
    //query wizard
    $data=$con->query("select * from designer_register where userid like '$userid' and password like '$password'");
    $row=mysqli_fetch_array($data);

    if($row[0]=="")
    {
       $flag=1;
       $data=$con->query("select * from designer_register where userid like '$_COOKIE[block]'");
       $row=mysqli_fetch_array($data);
                    ...
                    <form action="" method="post" name="login" id="loginform" >
						<div class="row">
    						...
    						<div class="form-group col-md-12 mt-0">
    							...
    							<p class="text-center m-0 ifc">Don’t you have an account?          <a href="designer_register.php" class="ia">Create Here</a></p>
    						</div>
                        </div>
					</form>
                </div>
